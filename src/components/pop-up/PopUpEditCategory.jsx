...
function PopUpEditCategory({ user_id, onClose, data, category_id }) {
  const [name, setName] = useState(data || data);
  ...
  const handleNewCategory = async () => {
    ...
    const data = await fetch(
      `/api/category?user_id=${user_id}&category_id=${category_id}`,
      config
    );
    ...
    try {
      ...
      toast.error("Update category unsuccessfull");
    }
  };
    ...
    <div className=" ease-in-out fixed inset-0 flex items-center justify-center bg-black/50">
      ...
      <div className=" bg-white p-8 rounded-lg  relative ">
        <div className="flex items-center  flex-col space-y-5 justify-center">
          <h1 className=" text-lg">Edit Cetagory</h1>
          <div className="mb-6">
            <input
              type="text"
              value={name}
